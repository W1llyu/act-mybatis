...
import act.mybatis.provider.MapperTemplate;
import act.mybatis.provider.EmptyProvider;
...
public class MapperHelper {
    ...
    private List<Class<?>> registerClass = Lists.newArrayList();
    private Map<Class<?>, MapperTemplate> registerProvider = new ConcurrentHashMap<Class<?>, MapperTemplate>();
    ...
    public void setProperties(Properties properties) {
        ...
        if (StringUtil.isNotEmpty(mapper)) {
            ...
            for (String mapperClass: mappers) {
                if (StringUtil.isNotEmpty(mapperClass)) {
    ...
    public void registerMapper(Class<?> mapperClass) {
        if (!registerProvider.containsKey(mapperClass)) {
            registerClass.add(mapperClass);
            registerProvider.put(mapperClass, fromMapperClass(mapperClass));
        }
        // 自动注册的继承的接口
    ...
    private MapperTemplate fromMapperClass(Class<?> mapperClass) {
        ...
        for (String methodName: methodSet) {
            try {
                ...
                throw new MapperException(templateClass.getCanonicalName() + "中缺少" + methodName + "(MappedStatement ms)方法!");
            }
        }
    ...
    public boolean isMapperMethod(String msId) {
        ...
        for (Map.Entry<Class<?>, MapperTemplate> entry : registerProvider.entrySet()) {
    ...
    public void ifEmptyRegisterDefaultInterface() {
        if (registerClass.size() == 0) {
            registerMapper("act.mybatis.mapper.Mapper");
        }
    }
